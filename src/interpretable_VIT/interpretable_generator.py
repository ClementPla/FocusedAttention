...
class LRP:
    ...
    def generate_LRP(self, input, index=None, method="transformer_attribution",
                     is_ablation=False, start_layer=0):
        ...
        return self.model.relprop(torch.tensor(one_hot_vector).to(input.device), method=method, is_ablation=is_ablation,
                                  start_layer=start_layer, **kwargs), output
...
def get_interpretable_ViT(state_dict,
                          to_cuda=True,
                          patch_size=16, embed_dim=1024, depth=24,
                          num_heads=16, img_size=384, num_classes=5, qkv_bias=True):
    network = VisionTransformer(patch_size=patch_size, embed_dim=embed_dim, depth=depth,
                          num_heads=num_heads, img_size=img_size, num_classes=num_classes, qkv_bias=qkv_bias)
